...
from models.age_estimation_pytorch.model import get_model
...
def def_age_estimate(gpu_ids=[]):
    pre_train_model = get_model(model_name="se_resnext50_32x4d", pretrained=None)
    pre_train_model.cuda(gpu_ids[0])
    pre_train_model.apply(weights_init('gaussian'))
...
class func(nn.Module):
    def __init__(self, style_dim, latent_dim, weight_norm, actvn, normalize_mlp):
        super(func, self).__init__()
        self.mlp1 = MLP(style_dim + 1, latent_dim, 256, 5, weight_norm=True, activation=actvn,
                        normalize_mlp=normalize_mlp)

    def forward(self, t, age_code):
        tt = torch.ones_like(age_code[:, :1]) * t
        ttx = torch.cat([tt, age_code], 1)
        out = self.mlp1(ttx)
        return out
...
class StyledDecoder(nn.Module):
    def __init__(self, output_nc, ngf=64, style_dim=50, latent_dim=256, n_downsampling=2,
        ...
        self.mlp1 = MLP(style_dim, latent_dim, 256, 5, weight_norm=True, activation=actvn, normalize_mlp=normalize_mlp)
        self.func = func(latent_dim, latent_dim, weight_norm=True, actvn=actvn, normalize_mlp=normalize_mlp)
        self.mlp2 = MLP(latent_dim, latent_dim, 256, 5, weight_norm=True, activation=actvn, normalize_mlp=normalize_mlp)

    def forward(self, id_features, target_age=None, traverse=False, deploy=False, interp_step=0.5,interv=20):
        t_list = np.linspace(0, 1.0, interv)
        options = {}
        ...
        if target_age is not None:
            if traverse:
                ...
                # temp_latent = self.mlp(target_age)  # original latent code
                first_latent = self.mlp1(target_age)
                latents = odesolve(self.func, first_latent, options=options)
                temp_np = latents.cpu().numpy()

                temp_latent=self.mlp2(latents[19])
                latent = temp_latent.new_zeros((output_classes, temp_latent.shape[1]))
            else:
                first_latent = self.mlp1(target_age)
                temp_latents = odesolve(self.func, first_latent, options=options)
                print('---------------')
                print('---------------')
                print('---------------')
                print('from 696')
                tl = temp_latents.detach().cpu().numpy()

                print(tl.shape)
                np.save("/home/yunzb/zt/age-ode/Lifespan_Age_Transformation_Synthesis/tl.npy",tl)
                print('-------------')
                print('save success')
                latents = []
                for i in range(interv):
                    latents.append(self.mlp2(temp_latents[i]))
                with torch.no_grad():
                    target_latent = self.mlp(target_age)
        ...
        if traverse:
            ...
            with torch.no_grad():
                for i in range(interv):
                    out1 = self.StyledConvBlock_0(id_features, latents[i])
                    out1 = self.StyledConvBlock_1(out1, latents[i])
                    out1 = self.StyledConvBlock_2(out1, latents[i])
                    out1 = self.StyledConvBlock_3(out1, latents[i])
                    out1 = self.StyledConvBlock_up0(out1, latents[i])
                    out1 = self.StyledConvBlock_up1(out1, latents[i])
                    out1 = self.conv_img(out1)
                    out.append(out1)

                ###########
                # out = self.StyledConvBlock_0(id_features, target_latent)
                # out = self.StyledConvBlock_1(out, target_latent)
                # out = self.StyledConvBlock_2(out, target_latent)
                # out = self.StyledConvBlock_3(out, target_latent)
                # out = self.StyledConvBlock_up0(out, target_latent)
                # out = self.StyledConvBlock_up1(out, target_latent)
                # out = self.conv_img(out)
                ###########

        return out, target_latent, latents[19]
...
class Generator(nn.Module):
    ...
    def forward(self, input, target_age_code, cyc_age_code, source_age_code, disc_pass=False):
        ...
        rec_latent_terminal = None
        rec_latent_ode = None
        if disc_pass:
            rec_outs = None
        else:
            rec_outs, rec_latent_terminal, rec_latent_ode = self.decode(orig_id_features, source_age_code)
        gen_outs, gen_latent_terminal, gen_latent_ode = self.decode(orig_id_features, target_age_code)
        fake_id_features = []
        fake_age_features=[]
        if disc_pass:
            ...
            with torch.no_grad():
                # fake_age_features = self.age_encoder(gen_outs[3])
                # fake_id_features = self.id_encoder(gen_outs[3])
                for ob in gen_outs:
                    fake_id_features .append(self.id_encoder(ob))
                    fake_age_features.append(self.age_encoder(ob))
            cyc_outs, _, _ = self.decode(fake_id_features[19], cyc_age_code)
        return rec_outs, gen_outs, cyc_outs, orig_id_features, orig_age_features, fake_id_features, fake_age_features, rec_latent_terminal, rec_latent_ode, gen_latent_terminal, gen_latent_ode
