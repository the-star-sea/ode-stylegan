...
import age_estimation_pytorch.age_estimate as age_estimate
import os
...
class LATS(BaseModel):  # Lifetime Age Transformation Synthesis
        ...
        for i, ob in enumerate(images):
            ...
            for j in range(ob.shape[0] / 2):
                ...
                if i == 0:
                    ...
                    age_bias_loss += self.age_bias_loss(delta_age, i / len(images))
    ...
    def update_G(self, infer=False):
        ...
        self.ca_age_bias(rec_images, self.class_A, self.class_B, age_bias_loss)
        self.ca_age_bias(gen_images, self.class_B, self.class_A, age_bias_loss)
        self.ca_age_bias(cyc_images, self.class_A, self.class_B, age_bias_loss)
        ...
        loss_dict = {'loss_G_Adv': loss_G_GAN.mean(), 'loss_G_Cycle': loss_G_Cycle.mean(),
                     ...
                     'loss_G_age_reconst': loss_G_age_reconst.mean(), "loss_G_latent": loss_G_latent.mean(),"age_bias_loss":age_bias_loss.mean()}
            ...
            if self.traverse or self.deploy:
                ...
                for i in range(self.numClasses):
                    ...
                    if self.isTrain:
                        ...
                        # st()
                        self.fake_B[i, :, :, :, :] = self.netG.infer(self.reals, self.gen_conditions)[0]
                    ...
                    if self.isTrain:
                        ...
                        self.cyc_A[i, :, :, :, :] = self.netG.infer(cyc_input, self.cyc_conditions)[0]
