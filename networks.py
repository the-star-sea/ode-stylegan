...
class func(nn.Module):
    def __init__(self, style_dim, latent_dim,weight_norm, actvn, normalize_mlp):
        ...
        self.mlp1 = MLP(style_dim + 1, latent_dim, 256, 3, weight_norm=True, activation=actvn,
...
class StyledDecoder(nn.Module):
    def __init__(self, output_nc, ngf=64, style_dim=50, latent_dim=256, n_downsampling=2,
        ...
        self.mlp1 = MLP(style_dim, latent_dim, 256, 3, weight_norm=True, activation=actvn, normalize_mlp=normalize_mlp)
    ...
    def forward(self, id_features, target_age=None, traverse=False, deploy=False, interp_step=0.5):
        ...
        if target_age is not None:
            if traverse:
                ...
                latent=odesolve(self.func, first_latent, options=options)
                latents=[]
                for i in range (10):
                    latents.append(self.mlp2(latent[i]))
        ...
        if traverse:
            ...
            for i in range (10):
                ...
                out.append(out1)
        return out,target_latent,latents[0]
...
class Generator(nn.Module):
    ...
    def forward(self, input, target_age_code, cyc_age_code, source_age_code, disc_pass=False):
        ...
        if disc_pass:
            ...
            rec_out,source_latent1,source_latent2 = self.decode(orig_id_features, source_age_code)

        gen_out,target_latent1,target_latent2 = self.decode(orig_id_features, target_age_code)
        fake_id_features =[]
        if disc_pass:
            ...
            fake_age_features=self.age_encoder(gen_out[0])
            for ob in gen_out:
                fake_id_features .append(self.id_encoder(ob))
            cyc_out,_,_ = self.decode(fake_id_features, cyc_age_code)
        return rec_out, gen_out, cyc_out, orig_id_features, orig_age_features, fake_id_features, fake_age_features,source_latent1,source_latent2,target_latent1,target_latent2
